<?php
    ...
    if ( isset($_POST['doorno']) ){
		...
		$shippingname = mysqli_real_escape_string($conn,$_POST['shippingname']);	
        $date = date('Y-m-d');
        // if (  !empty($doorno) && !empty($street) && !empty($city) && !empty($pincode) && !empty($country) ){
            $query = mysqli_query($conn, "UPDATE `users` SET `shippingContact1`='$shippingContact1', `mobile2`='$mobile2', `land`='$landnumber', last_updated='$date', doorno='$doorno', street='$street', city='$city', pincode='$pincode', country='$country', state='$state', shippingname='$shippingname' WHERE id='".mysqli_real_escape_string($conn,$_SESSION['SESS_DIT_UID'])."'") or die(mysqli_error($conn));
            if($query){
                $response = array();
                header("Content-Type: JSON");
                $data = mysqli_fetch_array($query);	
                $response["status"] = true;		
                $response["doorno"] = $doorno;
                $response["street"] = $street;
                $response["city"] = $city;
                $response["state"] = $state;
                $response["pincode"] = $pincode;
                $response["country"] = $country;
                $response["firstname"] = $shippingname;
